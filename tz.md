...
- **Система координат**: используем левостороннюю систему Babylon (значение по умолчанию). Совместимость обеспечивается единоразовой конверсией в мосте WASM→TS: инвертируем Z (`z = -z`). Не включаем `useRightHandedSystem`.
...
// Совместимость с астрономическими данными обеспечивается во входном мосту WASM→TS (единожды инвертируем Z)
