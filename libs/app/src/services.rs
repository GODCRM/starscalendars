...
impl AppServices {
    ...
    pub async fn new_production(
        ...
        let database_service = starscalendars_infra::DatabaseService::new(&config.database)
            .await
            .map_err(|e| {
                AppError::Configuration(format!("Database initialization failed: {}", e))
            })?;
        ...
        let cache_manager = starscalendars_infra::CacheServiceManager::new(&config.redis)
            .await
            .map_err(|e| {
                AppError::Configuration(format!("Redis cache initialization failed: {}", e))
            })?;

        let redis_cache = cache_manager.create_cache_service().await.map_err(|e| {
            AppError::Configuration(format!("Redis cache service creation failed: {}", e))
        })?;

        let telegram_cache = starscalendars_infra::TelegramCacheService::new(Box::new(redis_cache));
        ...
        let jwt_service = match (
            ...
            (Ok(private_path), Ok(public_path)) => {
                let private_key = std::fs::read(&private_path).map_err(|e| {
                    AppError::Configuration(format!("Failed to read private key: {}", e))
                })?;
                let public_key = std::fs::read(&public_path).map_err(|e| {
                    AppError::Configuration(format!("Failed to read public key: {}", e))
                })?;

                starscalendars_infra::JwtServiceImpl::new(&private_key, &public_key).map_err(
                    |e| {
                        AppError::Configuration(format!("JWT service initialization failed: {}", e))
                    },
                )?
            }
            _ => {
                info!("⚠️ Using generated JWT keys for development");
                starscalendars_infra::JwtServiceImpl::new_with_generated_keys().map_err(|e| {
                    AppError::Configuration(format!("JWT service initialization failed: {}", e))
                })?
            }
        };
        ...
        .await
        .map_err(|e| {
            AppError::Configuration(format!("Telegram service initialization failed: {}", e))
        })?;

        // Initialize repositories
        let user_repository =
            starscalendars_infra::PostgresUserRepository::new(database_service.pool().clone());

        let token_repository =
            starscalendars_infra::PostgresTokenRepository::new(database_service.pool().clone());
    ...
    pub fn new_test() -> Self {
        ...
        let telegram_cache =
            starscalendars_infra::TelegramCacheService::new(Box::new(in_memory_cache));
    ...
    pub async fn health_check(&self) -> Result<ServiceHealthReport, AppError> {
        ...
        match self.cache_service.get("health_check").await {
            ...
            Err(e) => {
                ...
                report
                    .errors
                    .push(format!("Cache health check failed: {}", e));
            }
        }
        ...
        match self.jwt_service.create_access_token(&test_claims).await {
            Ok(token) => match self.jwt_service.validate_access_token(&token).await {
        ...
        match self.telegram_service.health_check().await {
            ...
            Err(e) => {
                ...
                report
                    .errors
                    .push(format!("Telegram health check failed: {}", e));
            }
        }

        report.overall_healthy =
            report.cache_healthy && report.jwt_healthy && report.telegram_healthy;
