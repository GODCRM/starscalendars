...
impl CelestialBody {
    const ALL: &'static [CelestialBody] = &[
        Self::Sun, Self::Moon, Self::Mercury, Self::Venus, Self::Earth,
        Self::Mars, Self::Jupiter, Self::Saturn, Self::Uranus, Self::Neptune, Self::Pluto
    ];
}
...
fn calculate_all_positions_pure_astro_rust(
    ...
    buffer.push(sun_pos.x); buffer.push(sun_pos.y); buffer.push(sun_pos.z);
    ...
    buffer.push(moon_pos.x); buffer.push(moon_pos.y); buffer.push(moon_pos.z);
    
    // Mercury using VSOP87 theory
    let (mercury_long, mercury_lat, mercury_r) = astro::planet::heliocent_coords(&astro::planet::Planet::Mercury, jd);
    let mercury_pos = ecliptic_to_cartesian(mercury_long, mercury_lat, mercury_r);
    buffer.push(mercury_pos.x); buffer.push(mercury_pos.y); buffer.push(mercury_pos.z);
    
    // Venus using VSOP87 theory
    let (venus_long, venus_lat, venus_r) = astro::planet::heliocent_coords(&astro::planet::Planet::Venus, jd);
    let venus_pos = ecliptic_to_cartesian(venus_long, venus_lat, venus_r);
    buffer.push(venus_pos.x); buffer.push(venus_pos.y); buffer.push(venus_pos.z);
    ...
    buffer.push(earth_pos.x); buffer.push(earth_pos.y); buffer.push(earth_pos.z);
    
    // Mars using VSOP87 theory
    let (mars_long, mars_lat, mars_r) = astro::planet::heliocent_coords(&astro::planet::Planet::Mars, jd);
    let mars_pos = ecliptic_to_cartesian(mars_long, mars_lat, mars_r);
    buffer.push(mars_pos.x); buffer.push(mars_pos.y); buffer.push(mars_pos.z);
    
    // Jupiter using VSOP87 theory
    let (jupiter_long, jupiter_lat, jupiter_r) = astro::planet::heliocent_coords(&astro::planet::Planet::Jupiter, jd);
    let jupiter_pos = ecliptic_to_cartesian(jupiter_long, jupiter_lat, jupiter_r);
    buffer.push(jupiter_pos.x); buffer.push(jupiter_pos.y); buffer.push(jupiter_pos.z);
    
    // Saturn using VSOP87 theory
    let (saturn_long, saturn_lat, saturn_r) = astro::planet::heliocent_coords(&astro::planet::Planet::Saturn, jd);
    let saturn_pos = ecliptic_to_cartesian(saturn_long, saturn_lat, saturn_r);
    buffer.push(saturn_pos.x); buffer.push(saturn_pos.y); buffer.push(saturn_pos.z);
    
    // Uranus using VSOP87 theory
    let (uranus_long, uranus_lat, uranus_r) = astro::planet::heliocent_coords(&astro::planet::Planet::Uranus, jd);
    let uranus_pos = ecliptic_to_cartesian(uranus_long, uranus_lat, uranus_r);
    buffer.push(uranus_pos.x); buffer.push(uranus_pos.y); buffer.push(uranus_pos.z);
    
    // Neptune using VSOP87 theory
    let (neptune_long, neptune_lat, neptune_r) = astro::planet::heliocent_coords(&astro::planet::Planet::Neptune, jd);
    let neptune_pos = ecliptic_to_cartesian(neptune_long, neptune_lat, neptune_r);
    buffer.push(neptune_pos.x); buffer.push(neptune_pos.y); buffer.push(neptune_pos.z);
    ...
    buffer.push(pluto_pos.x); buffer.push(pluto_pos.y); buffer.push(pluto_pos.z);
...
pub fn get_orbital_elements(planet_index: usize, julian_day: f64) -> *const f64 {
    ...
    ELEMENTS_BUFFER.with(|buffer| {
        ...
        let planet = match planet_index {
            ...
            7 => astro::planet::Planet::Neptune,
            _ => {
                console_log!("❌ Invalid planet index for orbital elements: {}", planet_index);
                return std::ptr::null();
            }
        };
...
pub fn get_lunar_illumination_fraction(julian_day: f64) -> f64 {
    ...
    astro::lunar::illum_frac_frm_ecl_coords(
        moon_ecl.long, moon_ecl.lat, sun_ecl.long,
        moon_dist_au, sun_dist_au
    )
}
...
#[wasm_bindgen]
pub fn apply_precession_ecliptic(longitude_rad: f64, latitude_rad: f64, jd_from: f64, jd_to: f64) -> *const f64 {
    thread_local! {
    ...
    PRECESS_BUFFER.with(|buffer| {
        ...
        let (corrected_long, corrected_lat) = astro::precess::precess_ecl_coords(
            longitude_rad, latitude_rad, jd_from, jd_to
        );
...
#[wasm_bindgen]
pub fn apply_precession_equatorial(ra_rad: f64, dec_rad: f64, jd_from: f64, jd_to: f64) -> *const f64 {
    thread_local! {
    ...
    PRECESS_EQ_BUFFER.with(|buffer| {
        ...
        let (corrected_ra, corrected_dec) = astro::precess::precess_eq_coords(
            ra_rad, dec_rad, jd_from, jd_to
        );
...
#[wasm_bindgen]
pub fn get_planetary_apparent_magnitude_muller(planet_index: usize, phase_angle_rad: f64, delta_au: f64, r_au: f64) -> f64 {
    let planet = match planet_index {
        0 => astro::planet::Planet::Mercury,
        1 => astro::planet::Planet::Venus,
        3 => astro::planet::Planet::Mars,
...
#[wasm_bindgen]
pub fn get_planetary_apparent_magnitude_84(planet_index: usize, phase_angle_rad: f64, delta_au: f64, r_au: f64) -> f64 {
    let planet = match planet_index {
        0 => astro::planet::Planet::Mercury,
        1 => astro::planet::Planet::Venus,
        3 => astro::planet::Planet::Mars,
...
pub fn calculate_solar_zenith_position(julian_day: f64) -> *const f64 {
    ...
    ZENITH_BUFFER.with(|buffer| {
        ...
        // 1. Get Sun's geocentric ecliptic position using pure astro-rust (VSOP87)
        let (sun_ecl, _sun_dist_km) = astro::sun::geocent_ecl_pos(julian_day);
        
        // 2. Calculate nutation corrections for maximum precision
        let (nut_long, nut_oblq) = astro::nutation::nutation(julian_day);
        
        // 3. Calculate mean and true obliquity of ecliptic
        let mean_oblq = astro::ecliptic::mn_oblq_IAU(julian_day);
        let true_oblq = mean_oblq + nut_oblq;
        
        // 4. Apply nutation correction to Sun's longitude
        let sun_corrected_long = sun_ecl.long + nut_long;
        
        // 5. Convert corrected ecliptic coordinates to equatorial coordinates
        // Using pure astro-rust coordinate transformation functions
        let sun_right_ascension = astro::coords::asc_frm_ecl(sun_corrected_long, sun_ecl.lat, true_oblq);
        let sun_declination = astro::coords::dec_frm_ecl(sun_corrected_long, sun_ecl.lat, true_oblq);
        
        // 6. Calculate mean sidereal time at Greenwich using pure astro-rust
        let mean_sidereal_time = astro::time::mn_sidr(julian_day);
        
        // 7. Apply nutation to get apparent sidereal time
        let apparent_sidereal_time = astro::time::apprnt_sidr(mean_sidereal_time, nut_long, true_oblq);
        
        // 8. Calculate solar zenith longitude
        // Solar zenith occurs where local sidereal time = right ascension
        // Longitude where LST = RA is: longitude = GST - RA
        let mut zenith_longitude_rad = apparent_sidereal_time - sun_right_ascension;
        
        // 9. Normalize longitude to [-π, π] range
        zenith_longitude_rad = astro::angle::limit_to_two_PI(zenith_longitude_rad);
        if zenith_longitude_rad > std::f64::consts::PI {
            zenith_longitude_rad -= 2.0 * std::f64::consts::PI;
        }
        
        // 10. Solar zenith latitude = Sun's declination
        let zenith_latitude_rad = sun_declination;
        
        // 11. Convert to degrees (astronomical convention: longitude West-positive)
        let zenith_longitude_deg = -zenith_longitude_rad.to_degrees(); // West-positive
        let zenith_latitude_deg = zenith_latitude_rad.to_degrees();
...
#[wasm_bindgen]
pub fn convert_ecliptic_to_equatorial(ecl_long_rad: f64, ecl_lat_rad: f64, julian_day: f64, apply_nutation: bool) -> *const f64 {
    thread_local! {
    ...
    EQUATORIAL_BUFFER.with(|buffer| {
        ...
        let obliquity = if apply_nutation {
            ...
            mean_oblq + nut_oblq // True obliquity
        } else {
            astro::ecliptic::mn_oblq_IAU(julian_day) // Mean obliquity
        };
...
pub fn get_version() -> String {
    format!("StarsCalendars WASM Pure astro-rust wrapper v{}", env!("CARGO_PKG_VERSION"))
}
...
mod tests {
    ...
    fn test_pure_astro_rust_sun_position() {
        let result = get_sun_position(JulianDay::J2000.as_f64(), false);
        assert!(!result.is_null());
    }
    
    #[test]
    fn test_pure_astro_rust_moon_position() {
        let result = get_moon_position(JulianDay::J2000.as_f64(), false);
        assert!(!result.is_null());
    }
    ...
    fn test_pure_astro_rust_planet_position() {
        ...
        let result = get_planet_position(0, JulianDay::J2000.as_f64());
        assert!(!result.is_null());
    }
    
    #[test]
    fn test_pure_astro_rust_pluto_position() {
        let result = get_pluto_position(JulianDay::J2000.as_f64());
        assert!(!result.is_null());
    }
    
    #[test]
    fn test_pure_astro_rust_nutation() {
        let result = get_nutation(JulianDay::J2000.as_f64());
        assert!(!result.is_null());
    }
    
    #[test]
    fn test_pure_astro_rust_mean_obliquity() {
        let result = get_mean_obliquity(JulianDay::J2000.as_f64());
        assert!(result > 0.0); // Should be positive angle in radians
    }
    
    #[test]
    fn test_pure_astro_rust_julian_century() {
        let result = julian_day_to_century(JulianDay::J2000.as_f64());
    ...
    fn test_pure_astro_rust_orbital_elements() {
        ...
        let result = get_orbital_elements(2, JulianDay::J2000.as_f64());
        assert!(!result.is_null());
    }
    
    #[test]
    fn test_pure_astro_rust_lunar_illumination() {
        let result = get_lunar_illumination_fraction(JulianDay::J2000.as_f64());
        assert!(result >= 0.0 && result <= 1.0); // Illumination should be 0-1
    }
    
    #[test]
    fn test_pure_astro_rust_precession() {
        let result = apply_precession_ecliptic(0.0, 0.0, JulianDay::J2000.as_f64(), JulianDay::J2000.as_f64() + 365.25);
        assert!(!result.is_null());
    }
    
    #[test]
    fn test_solar_zenith_position_calculation() {
        let result = calculate_solar_zenith_position(JulianDay::J2000.as_f64());
        assert!(!result.is_null());
        
        // Test that zenith position is within Earth's surface bounds
        unsafe {
            let zenith_coords = std::slice::from_raw_parts(result, 2);
            let longitude = zenith_coords[0];
            let latitude = zenith_coords[1];
            
            // Longitude should be within -180 to +180 degrees
            assert!(longitude >= -180.0 && longitude <= 180.0, "Solar zenith longitude out of bounds: {}", longitude);
            // Latitude should be within -90 to +90 degrees (Sun's declination range ~±23.4°)
            assert!(latitude >= -90.0 && latitude <= 90.0, "Solar zenith latitude out of bounds: {}", latitude);
            // At J2000.0, Sun's declination should be reasonable (around winter solstice)
            assert!(latitude.abs() <= 25.0, "Solar zenith latitude unreasonable at J2000: {}", latitude);
        }
    }
    
    #[test]
    fn test_mean_and_apparent_sidereal_time() {
        let mean_st = get_mean_sidereal_time(JulianDay::J2000.as_f64());
        let apparent_st = get_apparent_sidereal_time(JulianDay::J2000.as_f64());
        
        // Both should be in valid range [0, 2π]
        assert!(mean_st >= 0.0 && mean_st <= 2.0 * std::f64::consts::PI, "Mean sidereal time out of range: {}", mean_st);
        assert!(apparent_st >= 0.0 && apparent_st <= 2.0 * std::f64::consts::PI, "Apparent sidereal time out of range: {}", apparent_st);
        
        // Apparent should differ slightly from mean due to nutation
        let difference = (apparent_st - mean_st).abs();
        assert!(difference < 0.1, "Sidereal time difference too large: {}", difference); // Should be small correction
    }
    
    #[test]
    fn test_ecliptic_to_equatorial_conversion() {
        // Test conversion of ecliptic coordinates to equatorial
        let result = convert_ecliptic_to_equatorial(0.0, 0.0, JulianDay::J2000.as_f64(), false);
        assert!(!result.is_null());
        
        unsafe {
            let eq_coords = std::slice::from_raw_parts(result, 2);
            let right_ascension = eq_coords[0];
            let declination = eq_coords[1];
            
            // Right ascension should be in [0, 2π] range
            assert!(right_ascension >= 0.0 && right_ascension <= 2.0 * std::f64::consts::PI, "Right ascension out of range: {}", right_ascension);
            // Declination should be in [-π/2, π/2] range
            // Declination validation using astronomical bounds (±90 degrees = ±π/2 radians)
            // This is a validation check, not an astronomical calculation
            assert!(declination.abs() <= 1.5708, "Declination out of range: {}", declination);
        }
    }
}
